...
UV_HTTP_TIMEOUT=600 uv pip install -r tf/requirements.txt --index-strategy unsafe-best-match --no-build-isolation
    ...
    uv run train.py --cfg configs/8x384x12h.yaml 

    uv run train.py --cfg configs/8x384x12h-c2r2bk.yaml

    uv run train.py --cfg configs/8x384x12h-crbk.yaml

    uv run train.py --cfg configs/BT4-8x256x8h-crbk.yaml

    uv run train.py --cfg configs/8x384x12h-4res.yaml
...
cd lczero/tf
    ...
    python train.py --cfg configs/8x384x12h.yaml

    python train.py --cfg configs/8x384x12h-c2r2bk.yaml

    python train.py --cfg configs/BT4-8x256x8h-crbk.yaml

    python train.py --cfg configs/MM-TT-MM-TT.yaml
