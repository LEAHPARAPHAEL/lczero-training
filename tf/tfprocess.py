...
class TFProcess:
    def __init__(self, cfg):
        ...
        self.policy_d_opt = self.cfg['model'].get('policy_d_opt', self.policy_d_model)
        self.policy_d_soft = self.cfg['model'].get('policy_d_soft', self.policy_d_model)
        ...
        self.q_ratio = self.cfg['training'].get('q_ratio', 0.0)
        ...
        for block in self.blocks:
            if block in ['T', 'B', 'D']:
                ...
                dims = self.cnn_blocks_dims[self.cnn_blocks]
                if dims > self.max_x_filters:
                    self.max_x_filters = dims
                self.blocks_dims.append(dims)
                self.x_blocks += 1
                self.cnn_blocks += 1                
    ...
    def init_net(self):
        ...
        def get_policy_optimism_weights(value_target, value_pred, value_err_pred, strength=2.0):
            ...
            weights = tf.math.sigmoid((z_values - strength) * 5.0)
    ...
    def benchmark_vram(self, batch_splits: int = 1):
        """
        Executes a one-time forward and backward backpropagation profile step 
        to capture the true peak VRAM requested by the active model architecture.
        """
        device_name = 'GPU:0'
        print("\n" + "="*50)
        print("  LAUNCHING ONCE-OFF VRAM MEMORY HARNESS PROFILE  ")
        print("="*50)

        # 1. Warm up the execution graph engine to initialize cuDNN workspaces
        print("Warming up graph engine structures...")
        for _ in range(2):
            x, y, z, q, m, st_q, opp_probs, next_probs = next(self.train_iter)
            if self.strategy is not None:
                _, grads = self.strategy_process_inner_loop(x, y, z, q, m, st_q, opp_probs, next_probs)
                _ = self.strategy_apply_grads(grads, batch_splits * self.strategy.num_replicas_in_sync)
            else:
                _, grads = self.process_inner_loop(x, y, z, q, m, st_q, opp_probs, next_probs)
                _ = self.apply_grads(grads, batch_splits)

        # 2. Flush and reset virtual memory allocator metrics back to zero
        tf.config.experimental.reset_memory_stats(device_name)

        # 3. Process a full production step while tracking peak memory allocation
        print("Profiling backpropagation pass...")
        x, y, z, q, m, st_q, opp_probs, next_probs = next(self.train_iter)
        ...
        # 4. Extract and print memory telemetry info
        mem_info = tf.config.experimental.get_memory_info(device_name)
        peak_vram = mem_info['peak'] / (1024 ** 2)      # Convert to Megabytes
        current_vram = mem_info['current'] / (1024 ** 2)

        print("\n" + "="*50)
        print(f" VRAM TELEMETRY BREAKDOWN ({self.cfg['name']})")
        print("-"*50)
        print(f" > Active VRAM currently held: {current_vram:.2f} MB")
        print(f" > ABSOLUTE PEAK VRAM CONSUMED:  {peak_vram:.2f} MB")
        print("="*50 + "\n")
        
        # Reset once more to leave clean memory tables for the main training loop
        tf.config.experimental.reset_memory_stats(device_name)
    ...
    @tf.function()
    def process_inner_loop(self, x, y, z, q, m, st_q, opp_probs, next_probs):
        ...
        with tf.GradientTape() as tape:
            ...
            '''
            policy_opponent_loss = self.policy_loss_fn(opp_probs, policy_opponent) if policy_opponent is not None else tf.constant(0.)
            policy_next_loss = self.policy_loss_fn(next_probs, policy_next) if policy_next is not None else tf.constant(0.)

            mixed_target = z * (1.0 - self.q_ratio) + q * self.q_ratio
            # Value losses
            value_winner_loss, value_winner_err_loss, value_winner_cat_loss = self.value_losses_fn(
                mixed_target, value_winner)
            ...
            if self.wdl:
                mse_loss = self.mse_loss_fn(z, value_winner)
    ...
    @tf.function()
    def strategy_process_inner_loop(self, x, y, z, q, m, st_q, opp_probs, next_probs):
        metrics, new_grads = self.strategy.run(self.process_inner_loop,
                                               args=(x, y, z, q, m, st_q, opp_probs, next_probs))
        metrics = [
    ...
    def train_step(self, steps: int, batch_size: int, batch_splits: int):
        ...
        for batch_id in range(batch_splits):
            ...
            if self.strategy is not None: 
                metrics, new_grads = self.strategy_process_inner_loop(
                    x, y, z, q, m, st_q, opp_probs, next_probs)
            else:
        ...
        if self.update_lr_manually:
            self.orig_optimizer.learning_rate.assign(self.active_lr)
        if self.strategy is not None:
    ...
    @tf.function()
    def calculate_test_summaries_inner_loop(self, x, y, z, q, m, st_q, opp_probs, next_probs):
    ...
    @tf.function()
    def strategy_calculate_test_summaries_inner_loop(self, x, y, z, q, m, st_q, opp_probs, next_probs):
    ...
    def calculate_test_summaries(self, test_batches: int, steps: int):
        ...
        for _ in range(0, test_batches):
            #x, y, z, q, m, st_q, opp_probs, next_probs = next(self.test_iter)
            x, y, z, q, m, st_q, opp_probs, next_probs = next(self.test_iter)
            if self.strategy is not None:
    ...
    def calculate_test_validations(self, steps: int):
        ...
        for metric in self.test_metrics:
            metric.reset()
        for (x, y, z, q, m, st_q, opp_probs, next_probs) in self.validation_dataset:
            if self.strategy is not None:
                metrics = self.strategy_calculate_test_summaries_inner_loop(
                    x, y, z, q, m, st_q, opp_probs, next_probs)
            else:
    ...
    def mha(self, inputs, emb_size: int, d_model: int, num_heads: int, name: str, att_expansion: int=1, layer_idx = 0):
    ...
    def depthwise_ffn(self, inputs, emb_size: int, dff: int, mask, kernel_size, name: str, glu=False):
        if glu:
            activation = tf.keras.activations.get("swish")
        elif isinstance(self.ffn_activation, str):
            activation = tf.keras.activations.get(self.ffn_activation)
    ...
    def squeeze_excitation(self, inputs, channels, name, data_format='channels_first'):
        assert channels % self.se_ratio == 0

        # Pass data_format directly to pool channels safely from NCHW or NHWC streams
        ...
        # Instantiate your custom layer passing the active layout configuration down
        return du.ApplySqueezeExcitation(data_format=data_format)([inputs, excited])
    ...
    def mobilenet_cuda_block(self, x, channels: int, dff: int, kernel_size: int, name: str, mask=None):
    ...
    def x_block(self, x, channels: int, dff: int, name: str):
        ...
        flow = du.DepthwiseXLayer(dff, name=name + "/2/conv2d")(flow)
    ...
    def cnn_to_encoder(self, flow, current_channels, target_d_model, name):
    ...
    def _transition_flow(self, flow, prev_type, curr_type, prev_dims, curr_dims, name):
        ...
        if self._is_sequence(prev_type) and self._is_spatial(curr_type):
            flow = self.encoder_to_cnn(flow, target_channels=curr_dims, name=name + "_enc-cnn")
            
        elif self._is_spatial(prev_type) and self._is_sequence(curr_type):
            flow = self.cnn_to_encoder(flow, current_channels=prev_dims, 
                                       target_d_model=curr_dims, name=name + "_cnn-enc")
            
        elif self._is_spatial(prev_type) and self._is_spatial(curr_type) and prev_dims != curr_dims:
    ...
    def _build_stem(self, inputs, block_type, block_dims, name):
        """Builds the initial entry point of the network based on the first block type."""
        flow = inputs
        ...
        if block_type == 'R':
            ...
            if self.embedding_ffn == 'D':
                mask, kernel_size, _ = self._get_depthwise_params()
            ...
            pos_info_processed = tf.keras.layers.Dense(
                64*self.embedding_dense_sz, name=name+"input/preprocess")(pos_info_flat)
    ...
    def _get_depthwise_params(self):
        mask = self.depthwise_masks[self._depthwise_count] if self.depthwise_masks else None
        kernel_size = self.depthwise_kernels[self._depthwise_count]
        dff = self.cnn_dffs[self._depthwise_count] if self.cnn_dffs else None
        self._depthwise_count += 1
        return mask, kernel_size, dff

    def _apply_block(self, flow, block_type, block_dims, block_idx, name, training):
        block_attn, block_acts = None, {}

        if block_type == 'M':
            mask, kernel_size, dff = self._get_depthwise_params()
            flow = self.mobilenet_cuda_block(flow, channels=block_dims, dff=dff, 
                                         kernel_size=kernel_size, name=name + "_mobilenet", mask=mask)
                                         
        elif block_type == 'C':
            mask, kernel_size, dff = self._get_depthwise_params()
            flow = self.convnext_block(flow, channels=block_dims, dff=dff, 
                                       kernel_size=kernel_size, name=name + "_convnext", 
    ...
    def construct_net(self, inputs, name: str = ""):
        ...
        for block_idx, block_type in enumerate(self.blocks):
            ...
            if block_idx == 0:
                flow = self._build_stem(flow, block_type, block_dims, name)
                
            else:
                prev_type = self.blocks[block_idx - 1]
                prev_dims = self.blocks_dims[block_idx - 1]
                flow = self._transition_flow(flow, prev_type, block_type, prev_dims, block_dims, block_name)

            flow, block_attn, block_acts = self._apply_block(
                flow, block_type, block_dims, block_idx, block_name, training=True
            )
        ...
        policy_optimistic_st = policy_head(
            name="policy/optimistic_st", depth=self.policy_d_opt) if self.cfg['model'].get('policy_optimistic_st', False) else None

        policy_soft = policy_head(
            name="policy/soft", depth=self.policy_d_soft) if self.cfg['model'].get('soft_policy', False) else None
        policy_opponent = policy_head(
            name="policy/opponent", depth=self.policy_d_opponent) if self.cfg['model'].get('policy_opponent', False) else None
        policy_next = policy_head(
            name="policy/next", depth=self.policy_d_next) if self.cfg['model'].get('policy_next', False) else None
